...
const Family = () => {
    ...
    const getFamilyMemberProfiles = useCallback(async () => {
        ...
        const famMemDocs = await Promise.all(
            family.members.flatMap((memberId) =>
                memberId !== userId ? [getDoc(doc(db, FsCol.Profiles, memberId))] : []
            )
        );
    ...
    const exportFamilyDataJSON = async () => {
        ...
        const residenceDocs = await Promise.all(
            family.residences.map((residence) => getDoc(doc(db, FsCol.Residences, residence)))
        );
        ...
        const vehicleDocs = await Promise.all(
            family.vehicles.map((vehicle) => getDoc(doc(db, FsCol.Vehicles, vehicle)))
        );
        ...
        const familyData = {
            ...
            pets: family.pets,
            groceryListSections: family.groceryList
                .filter((item) => item.uid.startsWith(catSubcatKeySeparator))
                .map((item) => item.name),
                                            ...
                                            <Stack direction='row' justifyContent='space-between' align='center' mb={3}>
                                                ...
                                                {userId === family.headOfFamily && (
                                                    <Menu>
                                                        ...
                                                        />
                                                        <MenuList>
                                                            <MenuItem onClick={() => removeFamilyMember(memberProfile)}>
                                                                Remove from family
                                                            </MenuItem>
                                                        </MenuList>
                                                    </Menu>
                                            ...
                                            {userId === family.headOfFamily && (
                                                <Menu>
                                                    ...
                                                    <MenuList>
                                                        ...
                                                        <MenuItem onClick={() => removePet(pet)}>
                                                            Remove from family
                                                        </MenuItem>
                                                    </MenuList>
                                                </Menu>
            ...
            {userId === family.headOfFamily && (
                <Box mt={4}>
                    ...
                    <Checkbox
                        ...
                        onChange={(event) =>
                            updateFamilySettings({ showAllTransactionsOnCurrentMonth: event.target.checked })
                        }
                        size='lg'
                    >
                    ...
                    <Text fontSize='xs'>
                        Include all transactions in the current month&apos;s budget calculations even if they occurred
                        outside the current month. Disabling this will allow scrolling to past and future months to see
                        their transactions (note that your current income/allotted values and categories will be the
                        same across all months)
                    </Text>
                </Box>
            )}
            ...
            {userId === family.headOfFamily ? (
                <Stack spacing={5} mt={6}>
                    ...
                    <Box>
                        ...
                        <Text fontSize='xs'>
                            Export a copy of your current family data as JSON (still pretty readable to
                            non-programmers!), not including finance data which can be exported separately on that page
                        </Text>
                    </Box>
